...
export function CrisisPanel() {
  ...
  const activate = async () => {
    ...
    setActivating(true);
    try {
      const status = await apiClient.post<CrisisState>(
        "/admin/crisis/activate",
        { minutes: duration, reason },
      );
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <button
            ...
            style={{
              background: "linear-gradient(135deg, #E8413C, #991b1b)",
          ...
          <p className="text-xs text-muted-foreground">
            Suspension temporaire de l&apos;activité — notifie toutes les apps
            (clients, cuisinières, livreurs) via{" "}
            <code>POST /admin/crisis/activate</code>.
          </p>
        </CardContent>
      </Card>
        ...
        <CardContent>
          {incidents.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {incidents.map((i) => {
                  const active = new Date(i.endsAt).getTime() > Date.now();
                  return (
                    <TableRow key={i.id}>
